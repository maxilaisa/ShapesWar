...
This project is configured for deployment on Render using a single web service. The backend serves the frontend static files.
...
4. The build process will:
   ...
   - Build the frontend (install dependencies + build)
   - Start the backend server which serves the frontend
5. Set environment variable `PORT=3001` (already configured in render.yaml)

The backend server serves the React frontend from the `/frontend/dist` directory, so both frontend and backend are accessible from the same URL.
